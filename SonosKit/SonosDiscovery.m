    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [discover findControllers:^(NSArray *ipAddresses) {
            ...
            NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                ...
                NSDictionary *supportInfo = [responseDict valueForKeyPath:@"ZPSupportInfo"];
                ...
                if([players count] > 1) {
                    ...
                    NSString *group = player[@"group"];
                    NSString *ip = [[player[@"location"] stringByReplacingOccurrencesOfString:@"http://" withString:@""] stringByReplacingOccurrencesOfString:@"/xml/device_description.xml" withString:@""];
                    NSArray *location = [ip componentsSeparatedByString:@":"];
                    SonosController *controllerObject = [[SonosController alloc] initWithIP:[location objectAtIndex:0]];
